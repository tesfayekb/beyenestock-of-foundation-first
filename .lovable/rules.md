...
When producing or revising any architectural specification, design 
document, or technical reference under trading-docs/, the author 
MUST verify every concrete reference against the actual repository 
state at HEAD before finalization:

- Every `backend/<file>.py:NN` reference must be confirmed by 
  reading the file
- Every Supabase table/column reference must be confirmed against 
  the latest applied migration in supabase/migrations/
- Every commit hash reference must be confirmed via `git show <hash>`
- Every claim that "X is implemented" or "Y exists" must cite the 
  file/commit proving it

Specifications produced without this verification are INVALID until 
verified. A spec audit (per the spec verification template at 
trading-docs/08-planning/ai-architecture-audits/_template.md) is 
...
# Note: docs/02-security/ and docs/03-performance/ are foundation-layer docs.
# No trading-docs equivalents exist. These references are intentionally kept.
