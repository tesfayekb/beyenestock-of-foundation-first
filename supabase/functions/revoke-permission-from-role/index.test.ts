...
const FUNCTION_URL = `${SUPABASE_URL}/functions/v1/revoke-permission-from-role`;
...
Deno.test("revoke-permission-from-role: no auth → 401", async () => {
...
Deno.test("revoke-permission-from-role: superadmin → success + audit", async () => {
  const jwt = await getJwtForUser(TEST_SUPERADMIN_ID);
  const admin = createClient(SUPABASE_URL, SERVICE_ROLE_KEY);

  // Create test role + assign a permission to it
  const { data: testRole } = await admin.from("roles").insert({
    key: `test-revoke-${crypto.randomUUID().slice(0, 8)}`,
    name: "Test Revoke Role",
    is_base: false,
    is_immutable: false,
  ...
  await admin.from("role_permissions").insert({
    role_id: testRole!.id, permission_id: perm!.id,
  });
  ...
  try {
    ...
    console.log("Superadmin revoke-permission response:", JSON.stringify(body));
    ...
    assertEquals(mapping!.length, 0, "Permission mapping must be removed");

    // Verify audit log
    const { data: auditRows } = await admin
      .from("audit_logs")
      .select("*")
      .eq("action", "rbac.permission_revoked")
      .eq("target_id", testRole!.id)
      .order("created_at", { ascending: false })
      .limit(1);
    assertEquals(auditRows!.length, 1);
    console.log("✅ Permission revocation + audit verified");
  } finally {
...
Deno.test("revoke-permission-from-role: regular user → 403", async () => {
