...
| AI-SPEC-008 | OPRA Flow Alpha | PLAN-OPRA-001 | V0.3 | B (alpha-generation) |
...
**Audit:** P1.3.7 audit complete 2026-04-28; redline at `trading-docs/08-planning/ai-architecture-audits/AI-SPEC-008.md` (583 lines, 14 sections + §0 Pre-Audit Verification)

| # | Dimension | Finding | Source / Evidence |
|---|-----------|---------|-------------------|
| 1 | File/module exists | **NO.** Verified 2026-04-28 — `grep -rln "flow_alpha_engine\|FlowAlpha" backend/ backend_agents/` returns ZERO matches across production code (excluding `.venv` site-packages). Closest existing logic: `backend/databento_feed.py` (452 lines at HEAD; +5 lines from evidence pack snapshot 447 — **informational drift only**) — Layer 0 raw OPRA trade-stream consumer producing 3 Redis keys (`databento:opra:trades` / `databento:opra:gex_block` / `databento:opra:confidence_impact`); `backend_agents/flow_agent.py` (343 lines at HEAD) — closest relevant agent (brief generator producing `ai:flow:brief` Redis key from Unusual Whales / put-call ratios). **Class B B-AI-008-1 → recommended canonical filename:** `backend/flow_alpha_engine.py` (matches AI-SPEC-005 `vol_fair_value_engine.py` per `B-AI-005-1` and AI-SPEC-006 `meta_labeler_engine.py` per `B-AI-006-1` per-spec naming pattern). | AI-SPEC-008 audit `B-AI-008-1` + spec §0 + §4 + §12 line 549. |
| 2 | DB table/column | **NO** tables at HEAD. Verified 2026-04-28 — `grep -rln "opra_flow_feature_bars\|opra_flow_events" supabase/` returns ZERO matches across all 68 migrations + `backend/` + `frontend/`. **Class B B-AI-008-3 → V0.3 ship adds 1 new migration** for both tables with full DDL: `opra_flow_feature_bars` (21 NUMERIC columns for 20 production features + 1 derived `zero_dte_minus_1_3dte_pressure` per `B-AI-008-18`, `features_jsonb JSONB`, `data_quality_flags JSONB`, `lee_ready_inferred_share NUMERIC`, `unknown_aggressor_share NUMERIC`, **`calibration_eligible BOOLEAN NOT NULL DEFAULT false`** for cross-spec substrate hygiene); `opra_flow_events` (event_type CHECK constraint with sweep/block/burst, expiry/strike/right enums, aggressor_sign / direction / confidence). Recommended migration filename: `2026MMDD_add_opra_flow_substrate.sql`. | AI-SPEC-008 audit `B-AI-008-3` + spec §3 lines 195–221. |
| 3 | Redis keys | **Reads (existing Layer 0):** `databento:opra:trades` (verified producer `databento_feed.py:349` lpush, 300s TTL); `databento:opra:gex_block` and `databento:opra:confidence_impact` (verified producers downstream). **New writes (Layer 2 V0.3):** **Class B B-AI-008-2 → 4 keys: `opra:flow_alpha:latest` (TTL = 15 minutes; full feature snapshot JSONB), `opra:flow_alpha:bars:1m` (LTRIM 120 bars; rolling minute-bar archive), `opra:flow_alpha:bars:5m` (LTRIM 80 bars; rolling 5-minute-bar archive), `opra:flow_alpha:health` (TTL = 5 minutes; backpressure state JSONB with `state` / `lag_seconds` / `last_emit_at` / `unknown_aggressor_share`).** Verified 2026-04-28 — `grep -rn "opra:flow_alpha\|opra:bbo\|opra:flow:" backend/ backend_agents/` returns ZERO matches. **Layer 0 + Layer 2 coexistence variant per `B-AI-008-17`:** `databento:opra:*` namespace remains as Layer 0 raw-feed substrate (continues serving GEX engine via `databento_feed.py:349 lpush`); `opra:flow_alpha:*` namespace ships as Layer 2 alpha-feature output; cutover is therefore additive — no migration of `databento:opra:trades` consumers. | AI-SPEC-008 audit `B-AI-008-2` + `B-AI-008-17` + spec §3 lines 175–191. |
| 4 | Current behavior matches spec | **NO.** OPRA alpha extraction is not performed at HEAD. `databento_feed.py` parses raw trades but does NOT compute aggressor side (Lee-Ready inference per `B-AI-008-5`), NBBO age, customer_delta, sweep clusters, block trade distributions, or any of the 20 production features. Existing trade ingestion is for GEX computation only (downstream consumer in `gex_engine.py`). | AI-SPEC-008 audit `B-AI-008-4` + `B-AI-008-5` + `B-AI-008-6` + `B-AI-008-7`. |
| 5 | Future design? | **YES — proposed and audited.** P1.3.7 audit complete; spec §1 lines 22–76 enumerates 20 production features (`opra_premium_z_5m` through `flow_price_divergence_5m`); spec §1 lines 33–46 + 114–126 + 128–133 specify Lee-Ready aggressor inference + sweep cluster detection + block trade detection; spec §4 lines 249–264 specifies multi-mode backpressure handling (normal / degraded / stale); spec §5 lines 270–305 specifies Item 5 (8 features) + Item 6 (20 features + 1 quality flag per `A-AI-008-2` corrected count) + Governor (5/6-field compact summary per `A-AI-008-2`) integration shims; spec §6 specifies replay process + production acceptance gate. **`A-AI-008-2` → harmonize Spec §5 framing to `6-field JSON dictionary`** (matches the example dictionary verbatim). | AI-SPEC-008 audit §10 + spec sections 1, 4, 5, 6. |
| 6 | Governance conflict | **POTENTIAL CONFLICT with D-008** (Data Budget ~$150–200/month) confirmed via P1.3.7 audit `C-AI-008-3`. Spec §1 lines 33–46 Lee-Ready aggressor inference assumes NBBO ticks at sub-second cadence; spec §1 lines 114–126 sweep cluster detection assumes same-millisecond cluster reconstruction across exchanges; spec §1 lines 128–133 block trade detection assumes 95th-percentile rolling distributions. **Procurement decision distinct from D-008 budget governance.** Three resolution options per `C-AI-008-3`: (1) within D-008 budget — confirm sub-second NBBO + sweep tape availability at current pricing; (2) D-008 budget revision via new D-XXX (D-025 candidate); (3) spec amendment to relax requirements; **(4 — DEFAULT) pre-implementation operator gate** — V0.3 implementation cannot begin until subscription scope verification completes. **Folds into D-023 enrichment item (u)** — Item 8 authority boundary (spec authority over Databento subscription scope is operator-only; Item 8 specifies what it needs but does not procure). T-Rule 4: D-008 binding. **Additional governance contradiction**: locked spec §8 lines 444–462 (coexistence with role-narrowing) vs locked AI_BUILD_ROADMAP.md §3.9 line 372 (successor-replacement) for `flow_agent.py` migration policy — escalated as `C-AI-008-2`; default option 1 = coexistence with role-narrowing per locked spec §8 explicit text; folds into D-023 enrichment item (v). | AI-SPEC-008 audit `C-AI-008-2` + `C-AI-008-3` + approved-decisions.md D-008 (lines 48–51); MASTER_PLAN backlog item 18. |
| 7 | Authority level proposed | **advisory only V0.3** per `C-AI-008-4` default option 3 (explicit V0.3 advisory-only acceptance) — Item 8 V0.3 ships per spec scaffolding (engine module + 20 features + storage architecture + freshness substrate + Redis namespace + Supabase tables) but produces no calibration-grade results until chain-archive matures per `C-AI-004-4` cascade; Items 5, 6, Governor stay advisory on Item 8 outputs; production-binding deferred to V0.4+ calendar contingent on Item 6 V0.2 ship + chain-archive maturation per `C-AI-008-4`. **Item 8 produces measurements consumed by Items 5/6/Governor; Item 8 does NOT have authority to override Item 1 Governor veto, Item 6 Meta-Labeler skip, or Item 5 Vol Fair-Value Engine outputs** — producer-only authority preserves capital preservation invariant per architectural principle 1. | AI-SPEC-008 audit `C-AI-008-4` + spec §6 lines 395–407. |
| 8 | Calibration dependencies | **HIGH (transitive on chain-archive substrate via `C-AI-008-4`).** P1.3.7 audit complete; **cross-spec contract gaps identified per `C-AI-008-1`**: Item 8 §5 unilaterally claims 8-feature contract with Item 5 + 20-feature contract with Item 6; verified 2026-04-28 — Item 5 spec contains 2 abstract OPRA references only (lines 390 + 549; no field-name enumeration), Item 6 spec is fully silent on Item 8 as 6th producer surface; **default option 3 = DEFERRED BILATERAL CONFIRMATION** (preserves audit-trail clarity; bilateral confirmation deferred to next P1.3.5 + P1.3.6 amendment cycle). **Calendar-blocked on**: (a) Item 4 V0.1 ship + chain-archive substrate sufficiency per `C-AI-004-4`; (b) Item 6 V0.2 Layer 2 engine module ship per `B-AI-006-1`; (c) Items 5 + 6 cross-spec contract bilateral confirmation per `C-AI-008-1`. **Confirms cross-spec FK cascade theme in 6 audits.** **Folds into D-023 enrichment item (w)** — cross-spec producer→consumer contract ratification process governance. | AI-SPEC-008 audit `B-AI-008-11` + `B-AI-008-12` + `B-AI-008-13` + `B-AI-008-14` + `C-AI-008-1`. |
| 9 | Training contamination risk | **MEDIUM-HIGH.** OPRA trade ingestion is short-window (300s TTL on `databento:opra:trades`); training data for flow-alpha must be archived externally via new `opra_flow_feature_bars` Supabase table (per `B-AI-008-3`) with `calibration_eligible BOOLEAN NOT NULL DEFAULT false` flag for cross-spec substrate hygiene. **Item 8 is NOT a 6th producer surface for `calibration_eligible` flag at spec level** (theme stays at 5 surfaces: Items 2, 4, 5, 6, 10) BUT scheduled buildout SHOULD include the column for cross-spec hygiene per `B-AI-008-3`. **Quality flags emission per `B-AI-008-8`**: `lee_ready_inferred_share_5m` + `unknown_aggressor_share_5m` to `opra_flow_feature_bars` columns AND `opra:flow_alpha:latest` JSONB output enables downstream consumers to de-weight inferred aggregates; threshold parametric (default 0.30) operator-tunable in `system-state.md` `opra_flow_alpha.unknown_aggressor_share_threshold`. Bilateral with Spec §4 lines 260–262 (`unknown_aggressor_share > threshold → reduce feature confidence`). | AI-SPEC-008 audit `B-AI-008-3` + `B-AI-008-8`. |
| 10 | Implementation owner | Cursor. | T-Rule 1. |

**Notes:**
- Item 8's training-data archival need overlaps with AI-SPEC-004 (Replay Harness) — both need archived OPRA flow; **bilateral with `B-AI-008-11`** replay process which is calendar-blocked on Item 4 V0.1 + chain-archive substrate sufficiency per `C-AI-004-4` cascade; AI-SPEC-008 audit confirms Item 4 V0.1 + Item 5 V0.2 + Item 6 V0.2 + Item 10 V0.2 chain-archive substrate dependency cascade extends transitively to Item 8 V0.3 (4-spec → 5-spec extension).
- **Layer 0 + Layer 2 coexistence variant** per `C-AI-008-2` default option 1 + `B-AI-008-17`: `databento_feed.py` Layer 0 raw OPRA stream consumer + new `flow_alpha_engine.py` Layer 2 alpha-feature producer; **NO LAYER 1** observability scaffold exists between them — Item 8 jumps directly Layer 0 → Layer 2; coexistence default per `C-AI-008-2` resolution + `B-AI-008-17`; cutover is therefore additive — no migration of `databento:opra:trades` consumers; expands the Layer 1 / Layer 2 architectural template family from 3 variants (Items 10 + 5 + 6) to 4 variants. **Document this Layer 0/Layer 2 coexistence variant in §1 carry-forward as the 4th variant of the canonical retrofit pattern for AI-architecture surfaces.**
- **flow_agent narrowing per `C-AI-008-2` default option 1**: existing `backend_agents/flow_agent.py` (343 lines at HEAD; brief generator producing `ai:flow:brief` Redis key consumed by `synthesis_agent.py:93` + `main.py:1917`) **narrows to LLM summarizer** reading from `flow_alpha_engine` output (5/6-field Governor compact summary per `B-AI-008-15`); existing `ai:flow:brief` Redis key continues but content shifts from Unusual Whales / put-call ratio brief to narrative summary over Item 8's compact summary; preserves backward-compat with synthesis_agent input contract.
- **Freshness gate substrate `B-AI-008-16`** confirms 4th HIGH-IMPACT carry-forward #4 — `opra:flow_alpha:health` Redis key (TTL = 5 minutes, JSONB with `state` / `lag_seconds` / `last_emit_at` / `unknown_aggressor_share`) is producer for Item 8's freshness gate; `_safe_redis()` is dead code at HEAD; consumer wiring at Items 5/6/Governor sites for `opra:flow_alpha:*` keys absent. **Theme escalates from "open governance debt" to "load-bearing safety substrate" with Item 8 P1.3.7 audit confirmation** — Item 8's 5-second backpressure handling per Spec §4 lines 257–258 (`If lag > 5 seconds: downstream treats OPRA features as stale`) makes freshness substrate even more time-sensitive than Items 1/5/6 (60-second TTL Redis keys); without `_safe_redis()` caller wiring at Items 5/6/Governor consumer sites for `opra:flow_alpha:*` keys, stale OPRA features could silently inform Items 5/6/Governor decisions after Databento feed lag. **Theme is now safety-critical for the entire Cluster B alpha-generation tier.**
...
**P1.3.8 audit addendum (2026-04-29):** This row pre-dates the locked `ITEM_9_EXIT_OPTIMIZER_LOCKED.md` archived 2026-04-26 + per-item P1.3.8 audit `AI-SPEC-009.md` redline (771 lines; 4 Class A + 20 Class B + 6 Class C findings; **CLUSTER B AUDIT 4 of 4 — CLOSES GATE C OF PATH Y ACTIVATION PREREQUISITES**). Three Class A corrections to this row are surfaced as `A-AI-009-2`, `A-AI-009-3`, `A-AI-009-4` in `AUDIT_FINDINGS_REGISTER.md`:

- **`A-AI-009-2` Tier label drift:** Row above marks **Tier: V0.2**; locked spec ships V0.3 advisory + V0.4 production per spec §11 lines 588–611 (graduated tier framing). **Recommended correction:** harmonize tier label to **V0.3 advisory → V0.4 production** matching locked spec's most-operationally-binding label (mirrors AI-SPEC-008 P1.3.7 `A-AI-008-1` tier-label-drift correction pattern).
- **`A-AI-009-3` "HARD CONFLICT" framing inaccurate:** Row 6 framing "HARD CONFLICT with D-010 + D-011 + D-017" is now stale. Reality per spec §0 lines 13–19 + §16 line 779 + `C-AI-009-2` Layer 1 + Layer 2 COEXISTENCE default option 1: Item 9 V0.4 is constrained to **earlier-exit-only** (close/reduce only; cannot delay static exits per spec §11 line 624; cannot override constitutional gates D-005/D-010/D-011/D-017/D-022 per spec §0 line 15 + §8 lines 426–452). The relationship between Item 9 Layer 2 (`exit_optimizer_engine.py`) and `position_monitor.py` Layer 1 is **COEXISTENCE — Layer 1 static-exit safety net continues; Layer 2 close/reduce-only overlay** — not "HARD CONFLICT". **Recommended correction:** harmonize row 6 framing to "COEXISTENCE per spec §16 line 779 — Layer 1 static-exit safety net continues; Layer 2 close/reduce-only overlay subject to constitutional gates D-005/D-010/D-011/D-017/D-022 with post-hoc replay verification per `C-AI-009-6` default option (a)".
- **`A-AI-009-4` `_DEBIT_RISK_PCT` claim inaccurate:** §3 row 416 in this matrix claims `AI-SPEC-009 reads _DEBIT_RISK_PCT (lines 103-111) for sizing-aware exit timing`; reality per spec §6 (locked spec lines 266–368) is that Item 9 enumerates **5 strategies** explicitly (Iron Condor §6.1, Iron Butterfly §6.2, Put Credit Spread §6.3, Long Straddle §6.4, Debit Call Spread §6.5) — only 3 of which are in `_DEBIT_RISK_PCT` set (Iron Butterfly, Long Straddle, Debit Call Spread; Iron Condor + Put Credit Spread are credit strategies not in `_DEBIT_RISK_PCT`); the remaining 5 canonical strategies in `_DEBIT_RISK_PCT` enum at `risk_engine.py:103-111` (`debit_call_spread` already covered, `debit_put_spread`, `long_call`, `long_put`, `long_straddle` already covered, `calendar_spread`) are NOT enumerated in spec §6 — deferred per `B-AI-009-9` operator authorization 2026-04-29 to V0.3 ship scope finalization. **Recommended correction:** harmonize §3 row 416 claim to "AI-SPEC-009 enumerates 5 strategies in spec §6 (Iron Condor, Iron Butterfly, Put Credit Spread, Long Straddle, Debit Call Spread); 5 additional canonical strategies from `_DEBIT_RISK_PCT` enum at `risk_engine.py:103-111` deferred per `B-AI-009-9` operator authorization 2026-04-29 — extend §6 OR explicitly mark inheritance per `C-AI-009-4` operator decision".

**Updated row dimensions per P1.3.8 audit (cumulative cross-references):**
- **Row 1 update:** `position_monitor.py` confirmed at 858 lines with **11 distinct `exit_reason` values** (verified 2026-04-29 — V8/V9 grep): `time_stop_230pm_d010`, `time_stop_345pm_d011`, `emergency_backstop`, `watchdog_engine_silent`, `eod_reconciliation_stale_open`, `straddle_pre_event_exit`, `take_profit_debit_100pct`, `stop_loss_debit_100pct`, `take_profit_40pct`, `cv_stress_exit_d017`, `stop_loss_150pct_credit`. **Locked spec proposes new `backend/exit_optimizer_engine.py` per `B-AI-009-1` canonical name — Layer 2 calibration-grade engine producing `exit_optimizer_recommendations` Supabase table per `B-AI-009-17`.**
- **Row 2 update:** Spec §11 V0.3 build sub-item ships NEW table `exit_optimizer_recommendations(id BIGSERIAL PK, position_id UUID FK trading_positions(id), strategy TEXT, recommendation_action TEXT, exit_advantage_R NUMERIC, optimizer_confidence NUMERIC, ev_uncertainty_R_adjusted NUMERIC, normal_threshold NUMERIC, high_stress_threshold NUMERIC, executed BOOLEAN DEFAULT false, executed_action TEXT, exit_reason_layer_1 TEXT, recommended_at TIMESTAMPTZ, executed_at TIMESTAMPTZ, calibration_eligible BOOLEAN NOT NULL DEFAULT false, ...)` — POSSIBLE 6th producer surface for `calibration_eligible` flag (scheduled for substrate hygiene only; theme stays at 5 producer surfaces at spec level — `B-AI-009-17`).
- **Row 3 update:** Spec §9 lines 478–489 + 499–509 unilaterally claim 10-field contract with Item 5 `vol:*` namespace + 9-field contract with Item 8 `opra:flow_alpha:*` namespace; Items 5/6/8 specs ALL silent on Item 9 per `C-AI-009-1` 3-way pattern (V4+V5+V6 zero matches); only Item 10 ↔ Item 9 bilateral (V7 6+ matches; explicit `get_exit_optimizer_training_paths()` accessor per Item 10 spec line 407 + "V0.4 With Item 9 (Exit Optimizer)" sub-section per Item 10 spec line 583). **Default option 3 = DEFERRED BILATERAL CONFIRMATION** per `C-AI-009-1` (mirrors `C-AI-008-1`).
- **Row 6 update (per `A-AI-009-3` correction):** "HARD CONFLICT" framing replaced with "COEXISTENCE per spec §16 line 779" — `position_monitor.py` Layer 1 STAYS as static-exit safety net even after V0.4 promotion of `exit_optimizer_engine.py` Layer 2 per `C-AI-009-2` Layer 1 + Layer 2 COEXISTENCE default option 1. **Item 9 introduces 4th variant of architectural template family: Layer 1 + Layer 2 COEXISTENCE for safety-critical specs.**
- **Row 7 update:** Authority level per spec §11 V0.3 = **advisory-only** (close/reduce recommendations logged to `exit_optimizer_recommendations`; `position_monitor.py` Layer 1 retains execution authority); spec §11 V0.4 = **per-strategy production authority for close/reduce only** subject to `C-AI-009-3` + `C-AI-009-4` per-strategy independent gates with system-level minimum 200 trades + per-strategy 40-trade floor + per-strategy Wilson_lower_95(useful_exit_rate) > 0.30 + per-strategy adaptive_exit_value > 0 over rolling 60 sessions. **Authority Recovery / Demotion contract per `C-AI-009-5` operator decision (default option 1 — operator-mediated demotion only mirrors `C-AI-006-1`).**
- **Row 8 update:** Spec §9 lines 478–489 + 499–509 — Item 9 V0.3 reads 10 fields from Item 5 `vol:*` Redis namespace (rv_forecast_remaining, implied_move_remaining, iv_rv_ratio, skew_z, smile_asymmetry_score, smile_width_score, tail_width_score, engine_confidence, authority_level, forecast_error_recent) + 9 fields from Item 8 `opra:flow_alpha:*` Redis namespace (directional_pressure_5m, directional_pressure_accel, flow_persistence_15m, vol_demand_pressure_5m, aggressive_put_call_imbalance_5m, sweep_directional_pressure_1m, dealer_hedge_pressure_z_5m, flow_price_divergence_5m, unknown_aggressor_share_5m); Item 9 V0.4 promotion-gate evaluation reads from Item 4 `replay_eval_cases` substrate per `B-AI-009-13` calendar-blocked on `C-AI-004-4` chain-archive substrate disposition; counterfactual ledger consumption from Item 10 V0.4-with-Item-9 sub-stage `get_exit_optimizer_training_paths()` accessor per `B-AI-009-14`. **Calendar-blocker depth: 4 upstream specs (Items 4, 5, 8, 10) — slightly shallower than Item 6 V0.2 since Item 9 does NOT depend on Items 1 + 7 + Item 6 itself; Item 9 V0.4 promotion adds 1 more dependency on `paper_phase_criteria` system-level GLC per `C-AI-009-3` default option 1.**
- **Row 9 update:** Earlier-exit-only enforcement per `C-AI-009-6` default option (a) — post-hoc verification per spec author intent; operator ratifies acceptance of asymmetric-risk-window (a runtime bug could extend trades for an entire validation window before being caught — every open position during the bug window is exposed to extension; validation window = rolling 60 sessions per spec §11 line 603); operator monitors `exit_optimizer_recommendations.executed = true AND exit_reason != 'optimizer_advised'` queries for replay-detected violations. Layer 1 + Layer 2 COEXISTENCE per `C-AI-009-2` default option 1 means Layer 1 static-exit safety net preserves expectancy floor against Layer 2 bugs; pre-spec exits continue to be rule-based per `position_monitor.py` Layer 1 with no contamination risk for Layer 2 training (Layer 2 trains on Item 10 V0.4-with-Item-9 sub-stage `trade_counterfactual_cases` per `B-AI-009-14` filtered on `calibration_eligible = true` + `simulation_status = 'success'`).

**Cross-cutting view changes from P1.3.8 audit:**
- **Cross-spec FK dependency cascade theme** (§5 cross-spec themes in AUDIT_FINDINGS_REGISTER.md): Item 9 confirmed as the deepest cross-spec FK consumer in Cluster B (transitively reads from Items 1, 4, 5, 6, 8, 10 per spec §9 contracts + `paper_phase_criteria` for V0.4 promotion); Item 9 also serves as new FK provider for downstream Item 10 V0.2 `exit_optimizer_recommendations` consumer per spec line 407.
- **`_safe_redis()` dead code carry-forward #4 theme:** Item 9 P1.3.8 audit confirms 5th HIGH-IMPACT consumer per `B-AI-009-20`; theme escalates from "safety-critical for Cluster B alpha-generation tier" (P1.3.7 framing) to **"load-bearing component of Item 9 V0.4 earlier-exit-only commitment" (P1.3.8 framing — most consequential framing yet)** — without freshness gate, stale Items 5/6/8 readings could cause Item 9 to make exit decisions on snapshots that are hours old, directly violating "earlier exit only" (an exit decision based on stale premise might be later than what fresh data would have prescribed); the freshness substrate is a load-bearing component of the earlier-exit-only commitment, not just a generic safety theme.
- **Layer 1 / Layer 2 architectural pattern theme:** Item 9 P1.3.8 audit confirms 5th adopter and introduces the **4th variant of architectural template family: Layer 1 + Layer 2 COEXISTENCE for safety-critical specs** — distinct from Items 10/5 (Layer 1 retrofit), Item 6 (dormant Layer 1 + Layer 2 fresh ship), and Item 8 (Layer 0 + Layer 2 coexistence).
- **Cross-spec producer→consumer contract ratification process theme:** Item 9 P1.3.8 audit extends the pattern from 2-way (Item 8 ↔ Items 5+6) to 3-way (Item 9 ↔ Items 5+8) and reveals the only-bilateral-among-pairs pattern (Item 10 ↔ Item 9 is the sole bilateral pair across both confirming audits).
- **Authority Recovery automatic-vs-operator-mediated theme:** Item 9 P1.3.8 audit confirms 3rd audit affected — Items 5 + 6 + 9 all extend the established "operator-mediated demotion only" pattern per `C-AI-006-1` resolution.
- **Strategy-class taxonomy naming inconsistency theme:** Item 9 P1.3.8 audit confirms 7th audit confirmation; surfaces `earnings_straddle` (per `position_monitor.py:549` `straddle_pre_event_exit` + Evidence Pack §10.2 line 660) as POSSIBLE 11th canonical strategy outside the existing 10 — flagged for operator clarification at `B-AI-009-9` resolution.
