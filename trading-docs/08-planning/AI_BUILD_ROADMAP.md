# MarketMuse — AI Build Roadmap v1.9
**Owner:** tesfayekb
...
**Patch v1.7 → v1.8 applies Phase 1 factual-correction cycle from external pre-deployment audit (Cursor 2026-04-28, conducted against the Action 1 v1.0 prompt and surfacing that `AUDIT_DISPOSITION_PLAN.md` item (m)'s prior 8-strategy canonical-list assertion was factually wrong: the migration it cited enumerates 10 values, not 8, with two of the listed values appearing in zero production code (verified via exhaustive grep — pure documentation phantoms) and four real canonical values silently omitted; this Build Roadmap had inherited the wrong 8-strategy framing in 3 places). 3 substantive sites in this document corrected: (i) §3.4 sub-item 6 utility-label-semantics line — prior 8-strategy framing replaced with canonical-10 framing (qualifier `(NOT Item 6's 5-strategy subset per B-AI-006-12)` preserved); (ii) §3.8 sub-item 7 strategy-taxonomy line — same replacement (bilateral-with-Item-2-V0.1 note preserved); (iii) §8 register G-101 row description updated from prior 8-baseline framing to canonical-10 framing (other columns unchanged). Authoritative ground truth: `supabase/migrations/20260419_add_strategy_types.sql:11-22` (10 canonical strategy_type values; no later migration alters the constraint, verified during pre-deployment audit). 2-doc cleanup cycle in this round (DP v1.8 → v1.9 + this v1.7 → v1.8); Master ROI Plan stays at v2.0.3 (no canonical-list claim in that doc; verified). Build Roadmap settles at v1.8; trio commit-ready as Master Plan v2.0.3 + this v1.8 + DP v1.9.
**Patch v1.8 → v1.9 applies SUBSCRIPTION_REGISTRY.md cross-references to §6 data-improvements table (per PR `docs/subscriptions-ui-and-registry-v1` 2026-04-30, against HEAD `822996c`). 4 logical edits at 4 physical sites — all in §6 table rows: (i) 15A status updated from "Bundles into Phase 4B + 5C" to "✅ COVERED — Polygon News (bundled in Stocks Advanced) + NewsAPI free tier (operator subscription); see SUBSCRIPTION_REGISTRY.md"; (ii) 15B status updated from "IS Item 8 — primary build target" to "✅ IS Item 8 — primary build target. Substrate covered by Databento OPRA Standard (live + historical from 2013-04-01); see SUBSCRIPTION_REGISTRY.md."; (iii) 15D build phase updated from "Out-of-scope V0.1; deferred to V0.2+" to "Out-of-scope V0.1; eligible for un-defer at V0.2" + status updated from "Per Master ROI Plan §6 out-of-scope" to "✅ COVERED — Polygon Stocks Advanced provides at no extra cost. ... See SUBSCRIPTION_REGISTRY.md."; (iv) 15E status updated from "Per Master ROI Plan §6 out-of-scope" to "⏸️ DEFERRED V2+ per Master Plan §6 out-of-scope. NOT pursuing per SUBSCRIPTION_REGISTRY.md §4." No structural changes; no other table row touched (15C unchanged — covered by existing Polygon entitlement). 1-doc cleanup cycle in this round (only this Build Roadmap; Master Plan advances v2.0.4 → v2.0.5 + DP advances v1.9 → v1.10 in companion edit groups of same PR). Build Roadmap settles at v1.9; trio commit-ready as Master Plan v2.0.5 + this v1.9 + DP v1.10. Companion: SUBSCRIPTION_REGISTRY.md created in same PR; UI mirror at `src/pages/trading/SubscriptionsPage.tsx` updated in lockstep (Polygon split into Options Developer + Stocks Advanced; Databento cost $150 → $199 reflecting OPRA Standard tier).
**Status:** **READY-TO-MERGE v1.9** — Phase 2 Document 1; cumulative through v1.9: all v1.2 deferred cosmetic items fixed in v1.3 + cross-reference patched in v1.4 + 7 substantive items from Cursor v1.3 review applied in v1.5 + 3 O-2 cleanup items applied in v1.6 + 6 converged-state cleanup items applied in v1.7 + 3 Phase 1 factual-correction items applied in v1.8 + 4 SUBSCRIPTION_REGISTRY.md cross-reference items applied in v1.9 per operator standing rule. Commit-ready as part of trio + companion: Master Plan v2.0.5 + this v1.9 + Audit Disposition Plan v1.10 + SUBSCRIPTION_REGISTRY.md.
**No items deferred from this draft.** Per operator standing rule "fix-now over defer-to-later" because thread/agent handoffs are silent-drop surfaces that annotation alone cannot fully prevent. If post-commit verification surfaces new items, they will be fixed in a subsequent v1.10 produced before commit.
...
6. Per-strategy utility label semantics (canonical 10 strategies, NOT Item 6's 5-strategy subset per B-AI-006-12)
7. Migration scripts
...
7. Strategy taxonomy: canonical 10 strategies (NOT 5-strategy subset per B-AI-006-12) — bilateral with Item 2 V0.1
...
|---|---|---|---|
| **15A** | Real-time news headlines in prediction context | Phase 4B (Item 6 V0.2 — feature input) + Phase 5C (Item 11 — event detection) | ✅ COVERED — Polygon News (bundled in Stocks Advanced) + NewsAPI free tier (operator subscription); see `SUBSCRIPTION_REGISTRY.md` |
| **15B** | Options flow signals (block trades, sweeps) from Databento | Phase 4C (Item 8 OPRA Flow Alpha) | ✅ IS Item 8 — primary build target. Substrate covered by Databento OPRA Standard (live + historical from 2013-04-01); see `SUBSCRIPTION_REGISTRY.md`. |
| **15C** | VIX full term structure (9D/3M/6M) | Phase 3E (Item 5 V0.x — surface features sub-item 14) | Bundles into Phase 3E |
| **15D** | Cross-asset signals (HYG, TLT, DXY) | Out-of-scope V0.1; **eligible for un-defer at V0.2** | ✅ COVERED — Polygon Stocks Advanced provides at no extra cost. Per Master ROI Plan §6 out-of-scope at V0.1; un-defer eligible at V0.2 (Item 6 V0.3 features in Phase 6). See `SUBSCRIPTION_REGISTRY.md`. |
| **15E** | Retail sentiment indicators (Reddit/Twitter) | Out-of-scope V0.1; **deferred to V2+** per `deferred-work-register.md` | ⏸️ DEFERRED V2+ per Master Plan §6 out-of-scope. NOT pursuing per `SUBSCRIPTION_REGISTRY.md` §4. |
...
| G-101 | Item 6 5-strategy enum is strict subset of canonical 10 | Cursor gap scan §3.H | §3.8 sub-item 7 — strategy taxonomy fix | [ ] |
...
**Coverage check (v1.5/v1.6/v1.7/v1.8 inherit v1.2 restatement post Cursor v1.4/v1.5/v1.7 reviews; v1.6 + v1.7 are bounded cross-doc-version-cleanup cycles + v1.8 is a Phase 1 factual-correction cycle — no §8 register changes):** All G-N IDs that Cursor gap scan §3.A delegated to this document are present (G-1 through G-16 contiguous; G-14 holds AI-SPEC-016 LightGBM per H-1 + H-B; G-49/50/51/55/59 for data improvements; G-67 through G-73 for V0.2 promotion gates including G-71 placeholder per Cursor v1.0 H-2; G-93/98/99/100/101/102/103/104 for substrate items). All `polygon:*` orphans + Layer-1 cutover items mapped. Cross-table consistency verified post-Cursor-v1.1: §3 prose ↔ §4 promotion table ↔ §5 ROI table ↔ §8 register all show Item 3 V0.1 / Phase 3F / Tier 1 (R-1 + R-2 fixes); Item 13 Tier 4 / 6-12-months-post-V0.3 / no V0.1 (R-3 fix); Phase 3X freshness substrate as standalone Tier-1 substrate work distinct from Item 13 build (R-3 fix). **CR-1 phantom citation closed in v1.5:** §7 16B row + §3.4 Stage 1 sub-item 10 now correctly enumerated (G-55 cross-reference verified). **No silent drops detected post v1.5 cross-table sweep; v1.6 + v1.7 cleanup cycles + v1.8 factual-correction cycle introduced no §8 changes.** *(If Cursor v1.8 verification finds residual silent drops, this coverage check needs further restatement.)*
...
*End of AI Build Roadmap v1.8 — DRAFT commit-ready as part of Phase 2 deliverable trio (Master Plan v2.0.3 + this v1.8 + Audit Disposition Plan v1.9). All v1.2 deferred cosmetic items fixed in v1.3; v1.3→v1.4 patched stale cross-reference; v1.4→v1.5 closes the CR-1 silent drop (B-AI-010-8 / G-55 / 16B VIX spread width recalibration) plus 6 other items from Cursor v1.4 review; v1.5→v1.6 applies Cursor combined-round O-2 (Master Plan v2.0.1 → v2.0.2 reference update at status line + footer) + O-2-derived forward-pointer roll-forward; v1.6→v1.7 applies converged-state cleanup (cross-doc version updates to Master Plan v2.0.3 + DP v1.8 + forward-pointer roll + 3 H-1/H-2/H-3 carryover residuals at §6/§8/§8 that O-2 cleanup missed); v1.7→v1.8 applies Phase 1 factual-correction cycle (canonical-taxonomy fix per Cursor 2026-04-28 pre-deployment audit; 3 substantive sites at §3.4/§3.8/§8 G-101 + bookkeeping at §6/§8 + status/footer version-anchor rolls + DP cross-reference v1.8 → v1.9; 2-doc cleanup cycle, Master ROI Plan unchanged at v2.0.3) per operator standing rule (fix-now over defer-to-later applied symmetrically). Commit all 3 documents together.*
