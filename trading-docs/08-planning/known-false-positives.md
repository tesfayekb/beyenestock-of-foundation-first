...
| D-013 | regime_agreement almost never disagrees | RESOLVED | Fixed in T-ACT-023 | regime_lgbm now uses GEX zero-gamma level — genuinely independent from VVIX-based regime_hmm. D-021 now fires when feeds disagree. |
...
| D-017 | SPX /prev endpoint appends same daily close repeatedly during one session | LOW | Phase B | `_fetch_spx_close()` in polygon_feed.py uses the /prev endpoint which returns yesterday's close. During a session, every 5-min poll appends the same value to spx_history, so the 20-value history represents real distinct days only after 20 trading days of uptime. Within one day, all log_returns are zero → realized_vol=0 → `if rv_val > 0` guard skips the IV/RV filter for that day. IV/RV filter effectively delayed until second trading day. Fix in Phase B: use Polygon daily aggregate to fetch last 20 distinct daily closes, deduplicated by date. |
| D-018 | Partial exit does not increment session virtual_pnl | LOW | Phase B | `run_position_monitor` partial exit block in position_monitor.py reduces contracts and writes audit log, but does NOT add partial_pnl to trading_sessions.virtual_pnl. D-005 drawdown check therefore cannot see partial gains until position fully closes. session.virtual_trades_count also not incremented. Effect: session P&L display understates gains until full close. Fix in Phase B alongside contracts local variable stale value issue. |
| D-019 | contracts local variable stale after partial exit in same iteration | LOW | Phase B | After partial exit DB update reduces contracts to 4, the local `contracts` variable still holds 5. Subsequent checks (50% take-profit, stop-loss) use max_profit computed with original 5 contracts. Makes 50% take-profit threshold slightly harder to hit on the remaining 4-contract position. Mildly conservative — position runs slightly longer before full exit. Fix in Phase B: update `contracts = remaining_contracts` after DB update. |
| D-020 | IV/RV filter has no monitoring for filter rate | LOW | Monitor | The IV/RV filter target is 15-20% of sessions blocked. No counter tracks actual block rate. Monitor `no_trade_reason LIKE 'iv_rv_%'` in prediction outputs. If >25% of sessions show this reason after 10 paper sessions, lower threshold from 1.05 to 1.02. |
