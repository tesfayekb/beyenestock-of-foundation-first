# MarketMuse — Audit Findings Disposition Plan v1.10
**Owner:** tesfayekb
...
**Patch v1.9 → v1.10 applies C-AI-004-4 substrate-now-present flag per `SUBSCRIPTION_REGISTRY.md` 2026-04-30 confirmation of Databento OPRA Standard historical access (from 2013-04-01, 12+ yr depth). 1 substantive site in this document: §1.3 [C-AI-004-4] entry — purely additive 2026-04-30 substrate-status update callout inserted after the existing "Cited by AI Build Roadmap §0 ..." line; existing default-position option-3 framing preserved verbatim per scope discipline. Operator decision can ratify to **option 2 (calibration-grade replay)** at Action 5a / Action 6 governance session — actual ratification deferred (out-of-scope for this PR per operator scope decision). Cascades flagged: AI-SPEC-001 V0.2 paper-binding (≥200 cards via Item 4 replay harness), AI-SPEC-005 V0.2 calibration-grade vol fair-value engine, AI-SPEC-010 V0.2 Layer 2 calibration-grade counterfactual P&L. 1-doc cleanup cycle in this round (only this DP; Master Plan advances v2.0.4 → v2.0.5 + Build Roadmap advances v1.8 → v1.9 in companion edit groups of same PR per `docs/subscriptions-ui-and-registry-v1`). DP settles at v1.10; trio commit-ready as Master Plan v2.0.5 + Build Roadmap v1.9 + DP v1.10. Companion: `SUBSCRIPTION_REGISTRY.md` created in same PR; UI mirror at `src/pages/trading/SubscriptionsPage.tsx` updated in lockstep.
...
**Concurrent review note:** AI Build Roadmap advanced to v1.9 in the same PR as this DP v1.10 patch (companion edit groups in `docs/subscriptions-ui-and-registry-v1`). Footer + §0 invariant below use version-agnostic citation ("AI Build Roadmap [current published version]") to avoid stale references regardless of further version churn through commit.
**Status:** **READY-TO-MERGE v1.10** — Phase 2 Document 3 (companion to `MASTER_ROI_PLAN.md` v2.0.5 + `AI_BUILD_ROADMAP.md` v1.9 — trio + companion `SUBSCRIPTION_REGISTRY.md` commit-ready as PR `docs/subscriptions-ui-and-registry-v1`)
...
**Substrate status update (2026-04-30):** Chain archive substrate prerequisite is now in place via Databento OPRA Standard historical access (from 2013-04-01, 12+ yr depth). Operator confirmed active subscription per `SUBSCRIPTION_REGISTRY.md` 2026-04-30. **This unlocks option 2 (calibration-grade replay over historical data).** Default option 3 (V0.1 advisory-only acceptance, wait 6+ months for forward archive) is no longer the constrained path. **Recommended ratification: option 2 at Action 5a / Action 6 governance session.** Cascades: AI-SPEC-001 V0.2 paper-binding (≥200 cards via Item 4 replay harness), AI-SPEC-005 V0.2 calibration-grade vol fair-value engine, AI-SPEC-010 V0.2 Layer 2 calibration-grade counterfactual P&L. See `SUBSCRIPTION_REGISTRY.md` §7. Existing default-position text above is preserved verbatim per scope discipline (this update is purely additive); ratification to option 2 happens at Action 5a / Action 6, not in this PR.
