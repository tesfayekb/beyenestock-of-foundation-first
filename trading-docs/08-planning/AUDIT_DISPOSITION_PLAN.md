# MarketMuse — Audit Findings Disposition Plan v1.8
**Owner:** tesfayekb
...
**Concurrent review note:** AI Build Roadmap settled at v1.7 (Cursor converged-state cleanup applied alongside this DP v1.8 patch in same simultaneous 3-doc cleanup cycle). Footer + §0 invariant below use version-agnostic citation ("AI Build Roadmap [current published version]") to avoid stale references regardless of further version churn through commit.
**Status:** **DRAFT v1.8** — Phase 2 Document 3 (companion to `MASTER_ROI_PLAN.md` v2.0.3 + `AI_BUILD_ROADMAP.md` v1.7 — all three settled in converged-state cleanup cycle; trio commit-ready)
...
| **(m)** | AI-SPEC-005 §11 + AI-SPEC-006 §11 (`B-AI-005-9` + `B-AI-006-12`) | Canonical strategy-taxonomy enumeration (extends item (i)): "Items 2 + 5 + 6 + 10 MUST use canonical 8-strategy IDs (`iron_condor`, `iron_butterfly`, `put_credit_spread`, `call_credit_spread`, `cash_secured_put`, `covered_call`, `long_call`, `long_put`) matching DB CHECK constraint `20260419_add_strategy_types.sql:11-22` and `risk_engine.py:105-106` literal — not the `bull_debit_spread` / `bear_debit_spread` alternative" |
...
| **(r)** | AI-SPEC-006 §11 (`B-AI-006-12`) | Strategy taxonomy reconciliation for Item 6 (extends item (m)): Item 6 V0.2 ships utility-label calculators for canonical 8 (adds `U_CCS`, `U_CSP`, `U_CC`, `U_LC`, `U_LP` to spec's existing 5) |
...
| **B-AI-006-12** | Item 6 5-strategy enum is strict subset of canonical 8 — taxonomy inconsistency | §3.8 sub-item 7 + D-023 enrichment item (r) | Approve canonical 8-strategy taxonomy; Item 6 V0.2 ships utility calculators for all 8 |
...
*End of Audit Disposition Plan v1.8 — DRAFT commit-ready as part of Phase 2 deliverable trio (Master Plan v2.0.3 + Build Roadmap v1.7 + this v1.8). All 7 Cursor v1.0 review items + all 5 Cursor v1.1 review residuals + 2 Cursor v1.2 review items + 1 Cursor v1.3 review item (N-1) + 1 Cursor v1.4 review item (O-1) + 2 combined-round items (H-1 + N-1) + 3 v1.6 → v1.7 cleanup items (Cursor O-2 site 1 + N-1 concurrent-note refresh + O-2-derived forward-pointer roll-forward) + 4 logical converged-state cleanup items (cross-doc version updates + forward-pointer roll + concurrent-note refresh + cumulative-fix-count math correction) fixed in v1.1 + v1.2 + v1.3 + v1.4 + v1.5 + v1.6 + v1.7 + v1.8 per operator standing rule (fix-now over defer-to-later applied symmetrically across all bounded items including derived ones). **Cumulative tally (math-corrected from inherited v1.6 typo): 7 + 5 + 2 + 1 + 1 + 2 + 3 + 4 = 25 items fixed across 8 review cycles + 1 combined-round + 1 cleanup-cycle + 1 converged-state-cleanup-cycle.** Commit trio together.*
