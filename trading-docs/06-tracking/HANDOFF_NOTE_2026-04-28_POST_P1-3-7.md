...
4. **T-ACT-054 — cv_stress degenerate-input pattern** (cv_stress design memo, 2026-05-03; remediation pending): When `vvix_z=0 AND gex_conf=1.0` (both common defaults during warmup, zero-variance, or saturated OPRA flow), the formula at `prediction_engine.py:691-694` correctly produces 0/0/0. ~29.2% of cycles in the last 7 days persisted these silent zeros, neutralizing 2 active downstream consumers (no_trade gate at `prediction_engine.py:1008`, strategy_selector long-gamma override at `strategy_selector.py:176`) — and the cv_stress > 70 rule-based direction tilt is dormant when LightGBM v1 is loaded but would also be silenced if it were active.

**Convention going forward:** Missing or saturated upstream inputs MUST persist as `null` (Python `None` / SQL `NULL`) in derived-feature columns, NOT as zero or any other in-band value. Downstream consumers MUST explicitly handle null with conservative semantics (skip the cycle, fall back to a known-defensive default, log WARN). Track B establishes this convention for the silent-staleness vectors (T-ACT-046, both feed paths). T-ACT-054's cv_stress fix will extend the convention to derived features (Choice A NULL-on-degenerate-input semantics; pending operator design call → already chosen as Choice A but not yet implemented).

**Discipline pointer for future PRs:** When a PR introduces or modifies a derived-feature compute path, the reviewer must verify (a) what input combinations could produce degenerate outputs that look like "no signal," and (b) whether downstream consumers have explicit null-handling. If either is missing, the PR is incomplete — same review-discipline class as A.5's schema-code coupling check.
