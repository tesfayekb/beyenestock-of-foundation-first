...
### A.7 Silent-failure-class family — when "valid math, degenerate semantics" produces zero-class outputs that look benign in isolation but break downstream contracts (added 2026-05-03 from cv_stress design memo finding; convention pointer for future agents)
...
This family currently includes (2026-05-03):
...
**Convention going forward:** Missing or saturated upstream inputs MUST persist as `null` (Python `None` / SQL `NULL`) in derived-feature columns, NOT as zero or any other in-band value. Downstream consumers MUST explicitly handle null with conservative semantics (skip the cycle, fall back to a known-defensive default, log INFO/WARN once-per-process to avoid log noise). Track B established this convention for the silent-staleness vectors (T-ACT-046, both feed paths). T-ACT-054 extended it to derived features (Choice A NULL-on-degenerate-input semantics — implemented 2026-05-02).

**Discipline pointer for future PRs (sharpened 2026-05-02 via T-ACT-054 plan review):** When a PR introduces or modifies a derived-feature compute path, the reviewer must verify (a) what input combinations could produce degenerate outputs that look like "no signal," and (b) whether downstream consumers have explicit null-handling. If either is missing, the PR is incomplete — same review-discipline class as A.5's schema-code coupling check.

**Plan-review meta-lesson (2026-05-02, from T-ACT-054 implementation):** Independent READ-ONLY plan review is non-optional before EXECUTE on derived-feature changes. Cursor's review of Claude's draft plan caught **1 critical defect** (OR-logic vs. AND-logic degenerate gate that would have caused production regression by NULLing healthy cycles), **1 missed active consumer** (D-017 in `position_monitor.py`), and **3 propagation-boundary silent-coercion sites** that would have defeated Choice A's semantic distinction. The single-pass plan would have shipped a worse-than-status-quo regression. Going forward: any derived-feature change crossing >2 consumer boundaries MUST go through DIAGNOSE-FIRST + independent plan review before EXECUTE. The 14-test surface (vs. Claude's draft 5) including the regression test `test_gex_saturation_alone_does_not_null_cv_stress` enforces the AND-logic discipline at CI.

Ratified 2026-05-03 via Track B PR `docs/track-b-silent-staleness-and-governance`. Reinforced 2026-05-02 via T-ACT-054 implementation PR `fix/t-act-054-cv-stress-null-on-degenerate` (T-ACT-054 entry status updated from REMEDIATION-PENDING to DONE; plan-review meta-lesson added).

### A.8 Future additions

Reserved placeholder for the next governance-discipline lesson surfaced by future incident, audit cycle, or post-mortem. Append future discipline additions here as A.8, A.9, ... rolling forward.
