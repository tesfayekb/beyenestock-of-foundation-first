...
### A.9 Verification-surface discipline — Railway CLI log queries silently truncate multi-day windows (added 2026-05-12 from T-ACT-065 closure session)

**Pointer:** Full content in standalone note at `trading-docs/06-tracking/HANDOFF_NOTE_2026-05-12_RAILWAY_CLI_TRUNCATION.md` (created in same governance amendment PR as this A.9 entry).

**Summary (for inline reference):** Railway CLI 4.57.5 silently caps `railway logs --since <iso> --until <iso>` queries to a recent window without surfacing the truncation. The T-ACT-065 7-day evaluation initially returned all-zeros from the CLI; cross-validation via `trading_cycle_skipped` count (also 0) revealed the truncation, and Railway dashboard export provided the real data (38 VIX-family stale events vs CLI's "0"). **Rule:** every "X count is N" claim deserves an independent corroboration query against a DIFFERENT surface BEFORE being used as decision input. See the standalone note for the full six-foot-gun accounting and acceptable corroboration-pair table.

**Cross-references:** T-ACT-065 closure entry in TASK_REGISTER §14 (2026-05-12); A.8 (sibling discipline — "subscription/entitlement claims as present-day factual questions"); HANDOFF_NOTE_2026-05-12_RAILWAY_CLI_TRUNCATION.md (full content).

### A.10 Future additions

Reserved placeholder for the next governance-discipline lesson surfaced by future incident, audit cycle, or post-mortem. Append future discipline additions here as A.10, A.11, ... rolling forward.
